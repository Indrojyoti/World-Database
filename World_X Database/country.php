...
<body >
  ...
  <option value = 'country.php?order=Name&num=0'>Order by CountryName</option>
  <option value = 'country.php?order=Population&num=0'>Order by Population</option>
  <option value = 'country.php?order=SurfaceArea&num=0'>Order by SurfaceArea</option>
  <option value = 'country.php?order=IndepYear&num=0'>Order by  IndepYear </option>
</select>
 <a href='country.php?order=$order&num=$next'> <button>Next page</button></a>
    <form id='tfnewsearch' method='get' action='country_show.php'>

            <input type='hidden' name='catagory' value='country'>
            <input type='text' class='tftextinput' name='name' size='21' maxlength='120'><input type='submit' value='search' class='tfbutton'>
    </form>
  <div class='tfclear'></div>
  </div>";
    $rows = mysql_query('select * from country order by '. $order.' limit '.$pages*$limit.','.$limit);
    echo '<br>';
